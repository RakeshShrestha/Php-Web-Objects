...
class FileCache {
    ...
    public function __construct() {
        $cachedir = CLASS_PATH . 'cachetmp/';
		if (!is_dir($cachedir) or !is_writable($cachedir)) {
			throw new FileIOException('Cache directory must be a valid writeable directory.');
			return;
		}
    ...
    public function set($key, $data, $ttl = 180) {
        ...
        if (!@file_put_contents($file, $data)) {
            throw new FileIOException('Error writing data to cache file.');
        }
    }
